...
parser = OptionParser()
parser.add_option(
    "-c", "--conn", dest="conn", help="the db connection uri", metavar="CONN"
)
...
def migrate_idqual():
    ...
    for sp_id, sp_idqual in sp_results:
        for acc in accession_table.select(
            accession_table.c.species_id == sp_id
        ).execute():
    ...
    sp_results = select([species_table.c.id], species_table.c.id_qual == None)
    for acc in accession_table.select(
        accession_table.c.species_id.in_(sp_results)
    ).execute():
        v = [acc[c] for c in acc_cols]
        v.append(None)
        rows.append(v)
