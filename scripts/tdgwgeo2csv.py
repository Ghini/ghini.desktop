...
parser.add_option(
    ...
    "--directory",
    dest="directory",
    help="directory of WGS txt files",
    metavar="DIR",
)
...
class Row(dict):
    ...
    def __init__(
        self, id=None, name=None, tdwg_code=None, iso_code=None, parent_id=None
    ):
        super().__init__(
            id=id,
            name=name,
...
def convert_level2():
    ...
    for line in reader:
        r = Row(
            id=str(id_ctr),
            name=line["L2_region"],
            tdwg_code=line["L2_code"],
            iso_code=line["L2_ISOcode"],
        )
...
def convert_level3():
    ...
    for line in reader:
        r = Row(
            id=str(id_ctr),
            name=line["L3_area"],
            tdwg_code=line["L3_code"],
            iso_code=line["L3_ISOcode"],
        )
...
def convert_level4():
    ...
    for line in reader:
        ...
        r = Row(
            id=str(id_ctr),
            name=line["L4_country"],
            tdwg_code=line["L4_code"],
            iso_code=line["L4_ISOcode"],
        )
...
def convert_gazetteer():
    ...
    for line in reader:
        ...
        if (
            line["L3_code"] in converted_rows
            and converted_rows[line["L3_code"]]["name"] == line["Gazetteer"]
        ):
            continue
        elif (
            line["L4_code"] in converted_rows
            and converted_rows[line["L4_code"]]["name"] == line["Gazetteer"]
        ):
            continue

        # TODO: create two rows, one for the gazetteer data and one for the
        # kew data
        r = Row(id=str(id_ctr), name=line["Gazetteer"], tdwg_code=line["ID"])
