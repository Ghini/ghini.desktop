...
header = [
    ...
    "Locale",
    "Contacto",
    "N° De Autorización de Investigación:",
    "N° autorización de movilización",
...
binomial_with_authorship = re.compile(
    r"^([A-Z][a-z]+) (?:(cf|aff|cf\.|aff\.|\?) )?(×[ ]?)?([-a-z]+|sp\.?[ ]?[1-9]?)(?: (.*[A-Z].*))?( \?)?$"
)
...
for r in csv.reader(open(input_file_name)):
    ...
    if easting_key in obj and northing_key in obj:
        if utm_slice:
            obj["lat"], obj["lon"] = staale.utm_to_latlon(
