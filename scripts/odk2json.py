...
for item in sorted(items, key=lambda x: x["acc_no_scan"] or x["acc_no_typed"]):
    ...
    if item["location"]:
        ...
        if db_loc:
            ...
            locations_needed[plant["location"]] = {
                "object": "location",
                "code": plant["location"],
            }
    ...
    if item["species"]:
        ...
        accession["species"] = item["species"] = "{} {}".format(
            genus_epithet, species_epithet
        )
    ...
    if db_accession is None:  # this is a new accession
        ...
        if (
            not item["species"]
            or db_accession.species.str(remove_zws=True) == item["species"]
        ):
            accession = {}
        ...
        if db_genus is None:
            ...
            if db_species is None:
                species = {
                    "object": "taxon",
                    "rank": "species",
                    "epithet": species_epithet,
                    "ht-rank": "genus",
                    "ht-epithet": genus_epithet,
                }
    ...
    for pic_name in item.get("photo", []):
        ...
        note = {
            "object": "plant_note",
            "plant": "%(accession)s.%(code)s" % plant,
            "category": "<picture>",
            "note": pic_name,
        }
        objects.append(note)
...
with open(os.path.join(path, "odk-seen.json"), "w") as output:
    output.write("[")
    output.write(",\n ".join([json.dumps(obj) for obj in to_skip]))
    output.write("]")
