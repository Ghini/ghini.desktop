...
#                  metavar='SRC')
parser.add_option(
    "-f", "--force", dest="force", action="store_true", help="the source path"
)
...
def do_geography(filename):
    ...
    default_file = os.path.join(
        rootdir, "plugins", "plants", "default", "geography.txt"
    )
...
def do_source():
    ...
    detail_writer = create_writer(
        os.path.join(dst_path, "source_detail.txt"), detail_columns
    )
    detail_ids = set()
    ...
    collection_writer = create_writer(
        os.path.join(dst_path, "collection.txt"), collection_columns
    )
