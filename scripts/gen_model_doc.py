...
for table in sorted(db.metadata.table_iterator(), key=lambda x: x.name):
    ...
    for col in table.columns:
        s = "<b>{}</b>: {}".format(col.name, col.type)
        if len(col.foreign_keys) > 0:
            s += ", ForeignKey(%s)" % ", ".join(
                [
                    "{}.{}".format(f.column.table, f.column.name)
...
for mapper in sorted(_mapper_registry, key=lambda x: x.class_.__name__):
    ...
    s = (
        ...
        + "->"
        + (
            mapper.local_table.name is not None
            and "/%s" % mapper.local_table.name
            or ""
        )
        + (
            mapper.local_table
            and mapper.local_table.description
            or str(mapper.local_table)
        )
        + (mapper.non_primary and "|non-primary" or "")
    )
    ...
    props_str = ""
    for p in sorted(
        mapper.iterate_properties, key=lambda x: isinstance(x, ColumnProperty)
    ):
