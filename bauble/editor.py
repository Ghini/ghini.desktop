...
class NotesBrowser(gtk.VBox):
    ...
    def __init__(self, cls, notes):
        """
        The NotesBox provides a GUI for editing a list of notes on a
        mapped instance.  The instance should follow an informal interface
        ...
        self.init_treeview()
        self.treeview = self.widgets.notes_treeview
        ...
        self.treeview.connect_after("size-allocate", callback,
                                    self.widgets.notes_note_column,
                                    self.widgets.notes_note_cell)



    def cell_data_func(self, col, cell, model, treeiter, attr):
        note = model[treeiter][0]
        v = getattr(model[treeiter][0], attr)
    ...
    def init_treeview(self):
        ...
        cell = self.widgets.notes_date_cell
        col = self.widgets.notes_date_column
        col.set_cell_data_func(cell, self.cell_data_func, 'date')

        cell = self.widgets.notes_category_cell
        col = self.widgets.notes_category_column
        col.set_cell_data_func(cell, self.cell_data_func, 'category')

        cell = self.widgets.notes_name_cell
        col = self.widgets.notes_name_column
        col.set_cell_data_func(cell, self.cell_data_func, 'name')

        cell = self.widgets.notes_note_cell
        col = self.widgets.notes_note_column
        col.set_cell_data_func(cell, self.cell_data_func, 'note')
