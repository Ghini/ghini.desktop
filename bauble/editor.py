...
class GenericEditorView:
    ...
    def __init__(
        ...
        for widget_name, markup in list(self._tooltips.items()):
            try:
                ...
                logger.debug(
                    _(
                        "Couldn't set the tooltip on widget "
                        "%(widget_name)s\n\n%(exception)s"
                    )
                    % values
                )
    ...
    def run_entry_dialog(
        ...
        entry.connect("activate", lambda entry: d.response(Gtk.ResponseType.ACCEPT))
    ...
    def run_yes_no_dialog(self, msg, parent: Optional[Any] = None, yes_delay: int = -1):
    ...
    def on_dialog_close(self, dialog, event: Optional[Any] = None):
        """
        Called if self.get_window() is a Gtk.Dialog and it receives
        the close signal.
        """
    ...
    def on_window_delete(self, window, event: Optional[Any] = None):
        """
...
class MockView:
    ...
    index: Any
    selection: Any
    reply_entry_dialog: Any
    reply_yes_no_dialog: Any
    ...
    def combobox_init(self, name, values: Optional[Any] = None, *args) -> None:
...
class GenericEditorPresenter:
    ...
    def __get_widget_name(self, widget):
        return isinstance(widget, str) and widget or Gtk.Buildable.get_name(widget)
    ...
    def on_textbuffer_changed(
        ...
        if value is None:
            ...
            )  # False -> don't include hidden text
            # value = widget.get_text()
            value = value or None
        logger.debug(
    ...
    def on_numeric_text_entry_changed(self, widget, value: Optional[Any] = None):
    ...
    def on_non_empty_text_entry_changed(self, widget, value: Optional[Any] = None):
    ...
    def on_datetime_entry_changed(self, widget, value: Optional[Any] = None) -> None:
    ...
    def on_check_toggled(self, widget, value: Optional[Any] = None) -> None:
    ...
    def on_relation_entry_changed(self, widget, value: Optional[Any] = None) -> None:
        attr = self.__get_widget_attr(widget)
        logger.debug(
    ...
    def on_combo_changed(self, widget, value: Optional[Any] = None, *args) -> None:
    ...
    def has_problems(self, widget: Optional[Any] = None):
        """
    ...
    def remove_problem(self, problem_id, widget: Optional[Any] = None) -> None:
        """
    ...
    def assign_completions_handler(
        self, widget, get_completions, on_select=lambda v: v
    ):
...
class ChildPresenter(GenericEditorPresenter):
    ...
    view: Any = property(_get_view, _set_view)
...
class NoteBox:
    ...
    def __init__(
        ...
        :param presenter: The presenter managing the note logic.
        :param model: The note model instance.
        ...
        filename = os.path.join(utils.paths.lib_dir(), self.glade_ui)
        ...
        self.box.pack_start(notes_box, True, True, 0)

        # Set session and preferences
        ...
        utils.set_widget_value(
            self.widgets.category_comboentry, self.model.category or ""
        )
    ...
    def update_label(self) -> None:
        ...
        if self.model.note:
            ...
            note_str = f" : {note_str}"
            # note_str = f" : {utils.xml_safe(self.model.note).replace('\n', '  ')}"
            # note_str = f" : {utils.xml_safe(self.model.note).replace('\n', '  ')}"
            max_length = 25
            label.append(
                f"{note_str[:max_length-1]} …"
    ...
    def is_valid_note(cls, note):
        """Check if a given note is valid."""
        return True
...
class PictureBox(NoteBox):
    ...
    def on_activate_browse_button(self, widget, data: Optional[Any] = None) -> None:
...
class NotesPresenter(GenericEditorPresenter):
    ...
    def add_note(self, note: Optional[Any] = None):
