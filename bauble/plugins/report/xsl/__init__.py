...
class SpeciesABCDAdapter(ABCDAdapter):
    ...
    def get_Notes(self):
        ...
        for note in self.species.notes:
            notes.append(
                dict(
                    date=butils.xml_safe(note.date.isoformat()),
    ...
    def format(cls, objs, **kwargs):
        ...
        if not on_path(exe):
            butils.message_dialog(
                _(
                    'Could not find the command "%(exe)s" to '
        ...
        if not os.path.exists(filename):
            butils.idle_message(
                _(
                    "Error creating the PDF file. Please "
...
try:
    ...
    butils.message_dialog(
        "The <i>lxml</i> package is required for the " "XSL report plugin"
    )
else:
