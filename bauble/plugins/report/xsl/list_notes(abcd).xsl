...
<xsl:stylesheet
  ...
  version="1.0"
  >
  <!-- find | and split into blocks, recursive template -->
  <xsl:template name="spliter">
    <xsl:param name="text"/>
    <xsl:param name="divider">|</xsl:param>
    <xsl:choose>
      <xsl:when test="contains($text,$divider)">
        <fo:block>
          <xsl:value-of select="substring-before($text,$divider)"/>
        </fo:block>
        <xsl:call-template name="spliter">
          <xsl:with-param name="text" select="substring-after($text,$divider)"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
