...
%>Familia	Subfamilia	Tribu	Subtribu	Genero	Especie	Autor	CITES	Condición	Conservación	Cantidad	Habito
% for v in values:
<%
    genus = v.genus.genus
    ...
    for a in v.accessions:
        quantity += len(a.plants)
%>${v.genus.family}	${subfamiliad[tribusd[subtribus]]}	${tribusd[subtribus]}	${subtribus}	${genus}	${species_epithet}	${v.author or ''}	${v.cites or ''}	${v.condition or ''}	${v.conservation or ''}	${quantity}		${v.habit or ''}
% endfor
