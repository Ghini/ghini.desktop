...
%>Item	Familia	Subfamilia	Tribu	Subtribu	Genero	Especie	Autor	Fecha registro	CITES	Condición	Conservación	Documento de ingreso	Número de Doc.	No.Plantas	Situación	Habito
% for v in values:
<%
    ...
    doc_number = ([n.note for n in v.notes if n.category == 'factura'] + [''])[0]
    if doc_number != '':
        doc_type = 'factura'
    else:
        doc_type = ''
%>${v}	${v.species.genus.family}	${subfamiliad[tribusd[subtribus]]}	${tribusd[subtribus]}	${subtribus}	${genus}	${species_epithet}	${v.species.author or ''}	${v._created.isoformat()[:10]}	${v.species.cites or ''}	${v.species.condition or ''}	${v.species.conservation or ''}	${doc_type}	${doc_number}	${len(v.plants)}		${v.species.habit or ''}
% endfor
